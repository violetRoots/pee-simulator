...
namespace UnityFx.Outline
{
	...
	public class OutlineSettingsEditor : Editor
	{
		...
		private const string _renderEventPropName = "_outlineRenderEvent";
		...
		public override void OnInspectorGUI()
		{
			...
			var renderEventProp = serializedObject.FindProperty(_renderEventPropName);
			var renderEvent = (CameraEvent)renderEventProp.intValue;
			...
			EditorGUILayout.PropertyField(renderEventProp);
		...
		internal static void DrawSettingsInstance(Rect rc, SerializedProperty property)
		{
			...
			if (settingsProp.objectReferenceValue)
			{
				...
				var renderEventProp = obj.FindProperty(_renderEventPropName);

				EditorGUI.BeginDisabledGroup(true);
				DrawSettingsInternal(rc, colorProp, widthProp, intensityProp, cutoffProp, renderModeProp, renderEventProp);
			...
			else
			{
				...
				var renderModeProp = property.FindPropertyRelative(_renderModePropName);
				var renderEventProp = property.FindPropertyRelative(_renderEventPropName);
		...
		private static void DrawSettingsInternal(Rect rc, SerializedProperty colorProp, SerializedProperty widthProp, SerializedProperty intensityProp, SerializedProperty cutoffProp, SerializedProperty renderModeProp, SerializedProperty renderEventProp)
		{
			var renderMode = (OutlineRenderFlags)renderModeProp.intValue;
			var lineCy = EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing;
			var n = 5;
			...
			EditorGUI.PropertyField(new Rect(rc.x, rc.y + 4 * lineCy, rc.width, EditorGUIUtility.singleLineHeight), renderEventProp, OutlineEditorUtility.RenderEventContent);
